...
def call_worker2(i, mode, up_ratio, input_data_name, centroid, furthest_distance, centroid_points, furthest_distance_patches, centroid_patches):
    ...
    np.save(os.path.join(save_root_path, mode,'%d'%up_ratio,'%d.npy'%i),label_patches)
...
def call_worker(i, up_ratio_list, mode, input_data_name) : 
    ...
    np.save(os.path.join(save_root_path, mode, 'basic','%d.npy'%i),input_patches)
    np.save(os.path.join(save_root_path, mode,'basic','%d_centroid.npy'%i),centroid_patches)
    np.save(os.path.join(save_root_path, mode,'basic','%d_fd.npy'%i),furthest_distance_patches)
...
def build_dataset(up_ratio_list,mode):
    for up_ratio in up_ratio_list:
        os.makedirs(os.path.join(save_root_path, mode,'%d'%up_ratio), exist_ok = True)
        
    os.makedirs(os.path.join(save_root_path, mode, 'basic'), exist_ok = True)
    ...
    results = []
    for (i, input_data_name) in enumerate(input_data_path_list):
...
if __name__=='__main__':
    ...
    build_dataset(up_ratio_list,'test')
